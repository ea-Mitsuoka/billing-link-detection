...
- [x] そのプロジェクトで Billing Export を有効化する
  ...
  - エクスポート種類: 標準使用量のコスト
- [x] エクスポートテーブル名（`gcp_billing_export_v1_XXXXXX_YYYYYY_ZZZZZZ`）を控える
- [x] `terraform/terraform.tfvars` を更新する
  ```hcl
  billing_export_project_id = "ea-gcsales-billing-export"
  billing_export_table      = "gcp_billing_export_v1_XXXXXX_YYYYYY_ZZZZZZ"
  ```
...
- [x] 以下を追加・更新済み
  ...
  | **Secret** | `BILLING_EXPORT_TABLE` | `gcp_billing_export_v1_XXXXXX_YYYYYY_ZZZZZZ` 設定済み（テーブル名に請求先アカウントIDが含まれるため Secret で管理） |
...
- [ ] BigQuery コンソールで以下のクエリを実行し、データが存在することを確認する
  ...
  FROM `ea-gcsales-billing-export.billing_data.gcp_billing_export_v1_XXXXXX_YYYYYY_ZZZZZZ`
  ```
...
- [ ] 各アラートを手動発火してテスト通知を確認する
  ...
  gcloud scheduler jobs run alert-cost_surge_projects \
    --location=asia-northeast1 --project=${GCP_PROJECT_ID}

  gcloud scheduler jobs run alert-subscription_cost_surge \
    --location=asia-northeast1 --project=${GCP_PROJECT_ID}
  ```
...
- [x] 翌日の 02:00 JST 以降に日次バッチが自動実行されたことを確認する（2026-05-28 02:00 SUCCEEDED）
  ```bash
