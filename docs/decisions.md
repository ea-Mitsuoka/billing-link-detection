...
**採用**: SQL（BigQuery WHERE句）\
**却下**: Python

**採用理由（SQL）**

- エンジニア以外も読み書きできる
- 条件変更でDockerイメージの再ビルド・デプロイが不要
- BigQueryコンソールで直接実行・デバッグできる
- YAMLに書いた条件文字列をそのままWHERE句に埋め込める（変換層不要）

**却下理由（Python）**

- 条件変更のたびにコード修正 + 再デプロイが必要
- YAMLの条件文字列をPythonの評価式に変換する独自パーサーが必要になる
- デバッグが複雑
